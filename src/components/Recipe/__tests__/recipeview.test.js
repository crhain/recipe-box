...
describe("RecipeView", () => {
  ...
  it("shows servings that matches recipe passed to component as state", () => {
    expect(component.find(".recipe__servings").text()).toEqual(
      "Servings: " + mockRecipe.servings
    );
  });

  it("shows a description that matches recipe passed to component as state", () => {
    expect(component.find(".recipe__description").text()).toEqual(
  ...
  it("shows a list of ingredient li elements that show the text for each ingredient passed from state", () => {
    ...
    expect(ingredientListElements.length).toEqual(
      mockRecipe.ingredients.length
    );
    expect(
      ingredientListElements.everyWhere((el, i) => {
        let ingredientMatchText =
  ...
  it("shows a list of instruction li elements that show the text for each instruction from state", () => {
    ...
    expect(instructionListElements.length).toEqual(
      mockRecipe.instructions.length
    );
    expect(
      instructionListElements.everyWhere((el, i) => {
